package com.duke.common.cache.utils;
...
public class RedisUtils {
    ...
    public static void delete(String... key) {
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                ...
                redisUtils.redisTemplate.delete(Arrays.asList(key));
            }
        }
    }
    ...
    public static Set<String> keysPattern(String pattern) {
        Set<String> keys = redisUtils.redisTemplate.keys(pattern);
        return keys;
    }
    ...
    public static Object get(String key, Object defaultValue) {
        Object result = key == null ? null : redisUtils.redisTemplate.opsForValue().get(key);
        if (result == null) {
            result = defaultValue;
        }
        return result;
     ...
     * @return true成功 false 失败
     */
    public static boolean set(String key, Object value, long time, boolean coverExpireTime) {
        try {
            if (!coverExpireTime) {
                if (redisUtils.redisTemplate.hasKey(key)) {
                    time = redisUtils.redisTemplate.getExpire(key);
                }
            }
            if (time > 0) {
                redisUtils.redisTemplate.opsForValue().set(key, value, time, TimeUnit.SECONDS);
